...
const Body = () => {
    ...
    return allRestaurants?.length === 0 ? (<Shimmer />) : (
        <>
            <div className="pt-20 h-96 text-center my-8 p-2 bg-gray-200">
                <div className="text-6xl font-bold mb-5 text-blue-400 p-10"><h1>Looking for top Restauraunts near you?</h1></div>
                <div className="flex justify-center">
                    <input type="text" className="w-1/2 p-2 m-2 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" placeholder="search" value={searchInput} onChange={(e) => {
                        setSearchInput(e.target.value);
                    }}></input>
                    <button className="p-2 m-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-300" onClick={() => {
